...
def load_reference_data():
    ...
    ese_sites = []
    with open(os.path.join(interpreter_dir, "rescue_esesites.txt")) as f:
        for line in f:
            temp = ""
            for char in line:
                if char != "\n":
    ...
    ess_sites = []
    with open(os.path.join(interpreter_dir, "esssites.txt")) as f:
        for line in f:
            temp = ""
            for char in line:
    ...
    with open(os.path.join(interpreter_dir, "exons_in_genomic_hg19.txt"), "r") as csvfile:
        spamreader = csv.reader(csvfile)
        temp = []
        for row in spamreader:
            temp += [row]
    ...
    genomic_dna = ""
    with open(os.path.join(interpreter_dir, "NG_012232.1.txt")) as f:
        for char in f:
            if char != "\n" and char != "\t":
                genomic_dna += char
    ...
    reference_cdna = ""
    with open(os.path.join(interpreter_dir, "DMD_cDNA.txt")) as f:
        for line in f:
            reference_cdna += line
    ...
    return {
        ...
        "domains": domains,
        "genomic_positions1": genomic_positions1,
        "genomic_positions2": genomic_positions2,
        "genomic_positions3": genomic_positions3,
...
def _triage_modal(title, body, options=None, input_value=None, input_placeholder=None, help_html=None):
    triage = {"title": title, "body": body}
    if options:
        triage["options"] = options
    if input_value is not None or input_placeholder is not None:
        triage["input"] = {
...
def _genomic_bounds(genomic_positions):
    lows = []
    highs = []
    for item in genomic_positions:
        if len(item) < 3:
            continue
        low = min(item[1], item[2])
        high = max(item[1], item[2])
        lows.append(low)
        highs.append(high)
    return min(lows), max(highs)


def _strip_reference_ids(text):
    cleaned = re.sub(r"\bN[A-Z]_\d+\.\d+\b", "", text, flags=re.IGNORECASE)
    return cleaned


def _extract_refseq_id(text):
    match = re.search(r"\bN[A-Z]_\d+\.\d+\b", text, flags=re.IGNORECASE)
    if match:
        return match.group(0)
    return None


def _extract_genomic_numbers(text):
    cleaned = _strip_reference_ids(text)
    nums = [int(n) for n in re.findall(r"\d+", cleaned)]
    return nums


def _is_genomic_input(text, bounds38, bounds19, len_ref):
    upper = text.upper()
    if "G." in upper or "NC_000023" in upper or "CHRX" in upper or "GRCH" in upper:
        return True
    nums = _extract_genomic_numbers(text)
    if not nums:
        return False
    if max(nums) > len_ref and (
        any(bounds38[0] <= n <= bounds38[1] for n in nums)
        or any(bounds19[0] <= n <= bounds19[1] for n in nums)
    ):
        return True
    return False


def _map_genomic_position_to_cdna(pos, exon_positions, genomic_positions):
    # First, try to map directly within an exon.
    for line in exon_positions:
        exon = line[0]
        g_low = min(genomic_positions[exon][1], genomic_positions[exon][2])
        g_high = max(genomic_positions[exon][1], genomic_positions[exon][2])
        if g_low <= pos <= g_high:
            gen_base = genomic_positions[exon][2]
            cdna_pos = line[2] - (pos - gen_base) + 1
            return int(cdna_pos), 0

    # Otherwise, map to the nearest exon boundary and encode intronic offset.
    nearest = None
    for line in exon_positions:
        exon = line[0]
        g_low = min(genomic_positions[exon][1], genomic_positions[exon][2])
        ...
        # Boundary at exon start (cDNA start) -> negative offset if upstream (higher genomic coord)
        if pos > g_high:
            offset = pos - g_high
            candidate = (offset, line[1], -offset)
            if nearest is None or candidate[0] < nearest[0]:
                nearest = candidate

        # Boundary at exon end (cDNA end) -> positive offset if downstream (lower genomic coord)
        if pos < g_low:
            offset = g_low - pos
            candidate = (offset, line[2], offset)
            if nearest is None or candidate[0] < nearest[0]:
                nearest = candidate
    ...
    _, cdna_pos, signed_offset = nearest
    return int(cdna_pos), int(signed_offset)


def _map_genomic_positions_to_cdna(positions, exon_positions, genomic_positions):
    mapped = []
    intron_positive = []
    intron_negative = []
    for pos in positions:
        cdna_pos, offset = _map_genomic_position_to_cdna(pos, exon_positions, genomic_positions)
        mapped.append(cdna_pos)
        if offset > 0 and not intron_positive:
            intron_positive = [cdna_pos, offset]
        if offset < 0 and not intron_negative:
            intron_negative = [cdna_pos, offset]
    return mapped, [intron_positive, intron_negative]


def _adjust_genomic_nums(raw_nums, bounds):
    lower, upper = bounds
    inside = [n for n in raw_nums if lower <= n <= upper]
    below = [n for n in raw_nums if n < lower]
    above = [n for n in raw_nums if n > upper]
    allowed = len(inside) > 0 or (len(below) > 0 and len(above) > 0)
    if not allowed:
        return [], False
    adjusted = []
    for n in raw_nums:
        if n < lower:
            adjusted.append(lower)
        elif n > upper:
...
def _parse_intronic_range_segment(segment):
    parts = []
    for item in segment.split("_"):
        match = re.match(r"^(\d+)([+-])(\d+)$", item)
        if match:
            base = int(match.group(1))
            sign = match.group(2)
            offset = int(match.group(3))
            parts.append((base, sign, offset))
    return parts


def _parse_hgvs_intronic_range(mutation):
    compact = mutation.replace(" ", "")
    match = re.search(r"(?:NM_\d+\.\d+:)?c\.\(([^)]+)\)_\(([^)]+)\)([a-z]+)", compact, re.IGNORECASE)
    if not match:
        return None
    left_seg = match.group(1)
    right_seg = match.group(2)
    suffix = match.group(3).lower()
    left_parts = _parse_intronic_range_segment(left_seg)
    right_parts = _parse_intronic_range_segment(right_seg)
...
def interpret_variant(mut):
    if not mut:
        return {"status": "error", "success": False, "message": "No input provided."}
    user_input = mut.strip()
    if not user_input:
        return {"status": "error", "success": False, "message": "No input provided."}

    transcript = _extract_refseq_id(user_input) or "NM_004006.2"
    mut = _strip_reference_ids(user_input).strip()
    if mut.startswith(":"):
        mut = mut[1:].strip()

    data = load_reference_data()
    ucsc_info = data["ucsc_info"]
    ese_sites = data["ese_sites"]
    ess_sites = data["ess_sites"]
    exon_positions = data["exon_positions"]
    domains = data["domains"]
    genomic_positions1 = data["genomic_positions1"]
    genomic_positions2 = data["genomic_positions2"]
    genomic_positions3 = data["genomic_positions3"]
    genomic_dna = data["genomic_dna"]
    reference_cdna = data["reference_cdna"]
    NSFP = data["NSFP"]
    ...
    parsed_intronic_range = False
    intron_range = _parse_hgvs_intronic_range(mut)
    if intron_range:
        suffix = intron_range["suffix"]
        if suffix in ("delins", "del", "dup", "ins"):
            if suffix == "delins":
                mutype, search = "Deletion/Insertion", "delins"
            elif suffix == "del":
                mutype, search = "Deletion", "del"
            elif suffix == "dup":
                mutype, search = "Duplication", "dup"
            elif suffix == "ins":
                mutype, search = "Insertion", "ins"
        left_parts = intron_range["left"]
        right_parts = intron_range["right"]

        left_choice = left_parts[1] if len(left_parts) > 1 else left_parts[0]
        right_choice = right_parts[0] if len(right_parts) > 0 else right_parts[-1]

        left_base, left_sign, left_offset = left_choice
        right_base, right_sign, right_offset = right_choice

        num_list = [left_base, right_base]
        intron_positive = []
        intron_negative = []

        if left_sign == "+" and not intron_positive:
            intron_positive = [left_base, left_offset]
        if left_sign == "-" and not intron_negative:
            intron_negative = [left_base, -left_offset]

        if right_sign == "+" and not intron_positive:
            intron_positive = [right_base, right_offset]
        if right_sign == "-" and not intron_negative:
            intron_negative = [right_base, -right_offset]

        intron_list = [intron_positive, intron_negative]
        parsed_intronic_range = True

    bounds38 = _genomic_bounds(genomic_positions2)
    bounds19 = _genomic_bounds(genomic_positions3)
    genomic_input = _is_genomic_input(mut, bounds38, bounds19, len_ref)

    if genomic_input and mutype == "Invalid mutation":
        upper = mut.upper()
        if "DEL" in upper and "INS" in upper:
            mutype, search = "Deletion/Insertion", "delins"
        elif "DEL" in upper:
            mutype, search = "Deletion", "del"
        elif "DUP" in upper:
            ...
            mutype, search = "Insertion", "ins"
        elif ">" in upper or "POINT" in upper or "TRANS" in upper or "TO" in upper:
            mutype, search = "Point mutation", ">"

    if genomic_input and not parsed_intronic_range:
        raw_nums = _extract_genomic_numbers(mut)
        if len(raw_nums) == 0:
            return {"status": "error", "success": False, "message": "No genomic positions detected."}

        adjusted_nums, allowed = _adjust_genomic_nums(raw_nums, bounds38)
        use_hg19 = False

        if not allowed:
            adjusted_nums, allowed = _adjust_genomic_nums(raw_nums, bounds19)
            use_hg19 = True

        if not allowed:
            body = (
                Markup("The genomic coordinates provided do not fall within the DMD gene region.")
                + Markup("<br><br>Please verify the reference sequence and coordinates.")
            )
            triage = _triage_modal("Genomic coordinates out of range", body, input_placeholder="Enter a DMD variant")
            return {"status": "triage", "triage": triage}

        if use_hg19:
            mapped, intron_list = _map_genomic_positions_to_cdna(adjusted_nums, exon_positions, genomic_positions3)
        else:
            mapped, intron_list = _map_genomic_positions_to_cdna(adjusted_nums, exon_positions, genomic_positions2)
        num_list = [n for n in mapped if n is not None]

        if len(num_list) == 0:
            return {"status": "error", "success": False, "message": "No variant type or position detected."}
    elif not parsed_intronic_range:
    ...
    if (mutype == "Deletion" or mutype == "Duplication") and "c." not in mut and len(num_list) > 0 and not ex_input and "Nucleotide" not in mut and not genomic_input:
        if max(num_list) < 80 and not ex_input:
            ...
            body = (
                Markup("You entered: <span class='text-danger'>")
                + escape(mut)
                + Markup("</span><br><br>Did you mean the exon-based notation below?")
            )
            options = [
                {"label": f"Yes, use exon notation: {temp}", "value": temp, "class": "btn-success btn-lg"},
                {
                    "label": f"No, use nucleotide notation: (Nucleotide) {mut}",
                    "value": f"(Nucleotide) {mut}",
                    "class": "btn-danger btn-lg",
                },
            ]
            triage = _triage_modal("Clarify notation", body, options=options)
            return {"status": "triage", "triage": triage}
...
#Triage other common input errors (3 cases):
    ...
    if past_max or mutype == "Invalid mutation":
        ...
        if past_max:
            body = (
                ...
                + escape(mut)
                + Markup(
                    f"</span><br><br>The largest position ({max(num_list)}) exceeds the length of the "
                    "<i>DMD, Dp427m</i> coding sequence (11055 nt)."
                )
            )
            triage = _triage_modal(
                "Position out of range",
                body,
                input_placeholder="Enter your revised mutation",
            )
            return {"status": "triage", "triage": triage}

        #Numbers, but no key words:
        if mutype == "Invalid mutation" and not past_max and len(num_list) > 0:
            ...
            body = (
                Markup("You entered: <span class='text-danger'>")
                + escape(mut)
                + Markup("</span><br><br>Select the mutation type you intended.")
            )
            options = [
                {"label": tridel, "value": tridel, "class": "btn-success btn-lg"},
                {"label": tridup, "value": tridup, "class": "btn-success btn-lg"},
                {"label": tripoint, "value": tripoint, "class": "btn-primary btn-lg"},
                {"label": triins, "value": triins, "class": "btn-primary btn-lg"},
                {"label": tridelins, "value": tridelins, "class": "btn-primary btn-lg"},
            ]
            if max(num_list) < 80:
                options.insert(1, {"label": tridelex, "value": tridelex, "class": "btn-info btn-lg"})
                options.insert(3, {"label": tridupex, "value": tridupex, "class": "btn-info btn-lg"})

            triage = _triage_modal("Select mutation type", body, options=options)
            return {"status": "triage", "triage": triage}
    ...
    if mutype != "Invalid mutation" and len(num_list) == 0 and not intron_only:
        body = (
            ...
            + escape(mut)
            + Markup(
                f"</span><br><br>This mutation type ({mutype}) requires a position to be interpreted."
            )
        )
        prefill = f"{pm_pre}{search}{pm_change} of "
        triage = _triage_modal(
            "Missing position",
            body,
            input_value=prefill,
            help_html="*Note that the symbol '>' denotes point mutation*",
        )
        return {"status": "triage", "triage": triage}
            ...
            if len(num_list) != 0:
                ...
                if ("Insertion" in mutype or mutype == "Point mutation") and len(pm_change) == 0:
                    body = (
                        ...
                        + escape(mut)
                        + Markup(
                            "</span><br><br>This mutation type requires that you input the inserted nucleotides "
                            "(i.e. an A, T, G, C or combination thereof)."
                        )
                    )
                    prefill = "c."
                    if len(num_list) == 1:
                        prefill += str(num_list[0])
                    if len(num_list) > 1:
                        prefill += str(min(num_list))
                        prefill += "_"
                        prefill += str(max(num_list))
                    if mutype == "Point mutation" and pm_pre:
                        prefill += pm_pre
                    prefill += search
                    triage = _triage_modal(
                        "Missing inserted bases",
                        body,
                        input_value=prefill,
                    )
                    return {"status": "triage", "triage": triage}
...
def results():
    mut = request.form.get("mutation", "").strip()
    payload = interpret_variant(mut)
    if payload.get("status") == "triage":
        return render_template("index.html", triage=payload["triage"])
    if payload.get("status") != "ok":
        return render_template("index.html", success=False)
    return render_template("main.html", **payload["result"])
...
def api_interpret():
    ...
    if request.is_json:
        ...
        mut = str(payload.get("mutation", "")).strip()
    if not mut:
        mut = request.values.get("mutation", "").strip()

    payload = interpret_variant(mut)
    status = 200 if payload.get("status") in ("ok", "triage") else 400
    return jsonify(_stringify_payload(payload)), status

### BEGIN INTERPRETER FUNCTIONS
...
def HGVS(positions, positions2, mtype, search_term, leng, ex_input, intron_list, pm_change, pm_pre, genomic=False):
	"""
	...
	if len(positions)!=0:
		if leng > 30 and ex_input:
			notation = str(min(positions)), "_" ,str(max(positions)), search_term #update to new HGVS notation
		if not genomic:
				...
				if len(intron_list[0]) != 0 or len(intron_list[1]) != 0:
					if min(positions) in intron_list[0]:
						...
						if max(positions) in intron_list[1]:
							neg_offset = intron_list[1][1]
							notation = str(min(positions)),"+",intron_list[0][1],"_",str(max(positions)),"-",str(abs(neg_offset)), pm_pre, search_term, pm_change
					...
					if min(positions) in intron_list[1]:
						if max(positions) in intron_list[0]:
							neg_offset = intron_list[1][1]
							notation = str(min(positions)),"-",str(abs(neg_offset)),"_",str(max(positions)),"+",str(intron_list[0][1]), pm_pre, search_term, pm_change
						if max(positions) in intron_list[1] and len(intron_list[1])>1:
							neg_offset = intron_list[1][1]
							neg_offset2 = intron_list[1][3]
							notation = str(min(positions)),"-",str(abs(neg_offset)),"_",str(max(positions)),"-",str(abs(neg_offset2)), pm_pre, search_term, pm_change
						if max(positions) not in intron_list[0] and max(positions) not in intron_list[1]:
							neg_offset = intron_list[1][1]
							notation = str(min(positions)),"-",str(abs(neg_offset)),"_",str(max(positions)), pm_pre, search_term, pm_change
			...
			if len(positions) == 1:
				notation = str(positions[0]),pm_pre,search_term, pm_change
				notation2 = str(positions2[0]),pm_pre,search_term,pm_change
			elif len(positions) > 1:
				notation = str(min(positions)),"_",str(max(positions)),pm_pre,search_term,pm_change
				notation2 = str(min(positions2)),"_",str(max(positions2)),pm_pre,search_term,pm_change
			else:
				print("uncaught issue in genomic HGVS()")
	notation = ''.join(notation)
	notation2 = ''.join(notation2)
	hgvs += notation
	hgvs2 += notation2
	return hgvs, hgvs2
	...
	if test_length > 8:
		...
		for index, ref_item in enumerate(ref_score):
			if float(ref_item) > 3 or float(mut_score[index]) > 3:
				...
				if percent > 0:
					percent_html = f"<c style='color:red'>+{str('%.2f' % percent)}% </c>"
				if percent <= 0:
					percent_html = f"<c style='color:red'>-{str('%.2f' % percent)}% </c>"
				if change > 0:
					change_html = f"+{change}"
				if change != 0:
					affected += [donor_frags[index], ref_score[index], donor_mfrags[index], mut_score[index], "<b> Donor </b>", change_html, percent_html]
		i = 0
		acceptor_frags = []
		acceptor_mfrags = []
		...
		for index, ref_item in enumerate(ref_score):
			if float(ref_item) > 3 or float(mut_score[index]) > 3:
				...
				if change > 0:
					change_html = f"+{change}"
				if change != 0:
					affected += [donor_frags[index], ref_score[index], donor_mfrags[index], mut_score[index], "<b> Acceptor </b>", change_html, percent_html]
	if len(affected)==0:
		affected += ["False"]
	return affected
...
def cdna2gen(num_list, exon_positions, genomic_positions1, genomic_positions2, genomic_positions3, intron_list):
	"""
	Returns a list:
			...
			if num >= line[1] and num <= line[2]:
				...
				if len(intron_list[0]) != 0:
					if intron_list[0][0] == num and intron_list[0][1] != '?':
						gen_position = gen_position + intron_list[0][1] #+1 #+1 added 11/20/2017
				if len(intron_list[1]) != 0:
					if intron_list[1][0] == num and intron_list[1][1] != '?':
						gen_position = gen_position + intron_list[1][1] #+1 #+1 added 11/201/2017
	...
	for num in num_list:
		for line in exon_positions:
			if num >= line[1] and num <= line[2]:
				...
				if len(intron_list[1]) != 0:
					if intron_list[1][0] == num and intron_list[1][1] != '?':
						gen_position = gen_position - intron_list[1][1] #-1 #-1 added 11/20/2017
	...
	for num in num_list:
		for line in exon_positions:
			if num >= line[1] and num <= line[2]:
				exon = line[0]
				difference_3 = (line[2]) - num +1
				gen3 = genomic_positions3[exon][2]
				gen_position = gen3 + difference_3
				if len(intron_list[0]) != 0:
					if intron_list[0][0] == num and intron_list[0][1] != '?':
						gen_position = gen_position - intron_list[0][1] #-1 #-1 added 11/20/2017
				if len(intron_list[1]) != 0:
					if intron_list[1][0] == num and intron_list[1][1] != '?':
						gen_position = gen_position - intron_list[1][1] #-1 #-1 added 11/20/2017
				pos_gen3 += [gen_position]
	return pos_gen1, pos_gen2, pos_gen3  
	...
	if mutype == "Deletion":
		...
		if (length + right_length) % 3 == 0 and right_length != 0:
			...
			while i < (length + right_length)/3:
				filler += '-'
				i += 1
			aa_skipgap = aa_skip[0:floor(min(num_list)/3)] + filler + aa_skip[floor(min(num_list)/3):]
			end = min(num_list)
			if end%3 == 1:
				aa_skipgap = aa_skip[0:floor(end/3)] + filler + aa_skip[floor(end/3):]
			if end%3 == 2:
...
def aa_longform(aa, mutype, frame_shift, num_list, length_mutation, aa_ref):
	...
	lst = ['A','Ala','C','Cys','D','Asp','E','Glu','F','Phe','G','Gly','H','His','I','Ile','K','Lys','L','Leu','M','Met','N','Asn','P','Pro','Q','Gln','R','Arg','S','Ser','T','Thr','V','Val','W','Trp','Y','Tyr','*','*']																							 
	if mutype == "Deletion" and not frame_shift:
		first_position = floor(floor(min(num_list)/3))
		if min(num_list)%3==2:
			first_position += 1
		last_position = first_position + floor(length_mutation/3)
...
def ClinVar(mvr, hgvs):
	...
	if len(cv)>0:
		...
		cv_table = "Variant not found in ClinVar via myVariant.info, however, please <a href='https://www.ncbi.nlm.nih.gov/clinvar/?%s' target='_blank'> click here </a>to search ClinVar directly" % urlencode({"term": hgvs})
	return cv_table

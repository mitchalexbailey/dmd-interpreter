...
def results(request):
	...
	with open(os.path.join(interpreter_dir, "exon_positions.csv"),'rb') as csvfile:
	...
	#Domain positions
	with open(os.path.join(interpreter_dir, "domains.txt"),'rb') as csvfile:
		spamreader = csv.reader(csvfile)
		temp = []
		for row in spamreader:
			temp += [row]
	...
	with open(os.path.join(interpreter_dir, "exons_in_genomic_old.txt"), 'rb') as csvfile:
		spamreader = csv.reader(csvfile)
		temp = []
		for row in spamreader:
			temp += [row]
	...
	with open(os.path.join(interpreter_dir, "exons_in_genomic.txt"), 'rb') as csvfile:
		spamreader = csv.reader(csvfile)
		temp = []
		for row in spamreader:
			temp += [row]
	...
	with open("./interpreter/exons_in_genomic_hg19.txt", 'rb') as csvfile:
		spamreader = csv.reader(csvfile)
		temp = []
		for row in spamreader:
			temp += [row]
...
#Triage other common input errors (3 cases):
	...
	if not past_max and mutype != "Invalid mutation": # and len(num_list) != 0:       
			...
			if len(num_list) != 0:
				...
				if mutype == "Deletion" and len(num_list) != 0:
					...
					if frame_shift == False:
						...
						aa_seqgapped = aa_seq[start1:(end1/3)] + filler + aa_seq[(end1/3):]
		...
		if search == ">":
			pmindex = standard_hgvs.index(">")
			standard_hgvs_search = standard_hgvs[0:pmindex], "%3E",pm_change
